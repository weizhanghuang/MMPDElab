...
% find the indices of the elements attached to the boundary facets

   bfE = MovMesh_bfAttachments(X,tri,tri_bf);
...
% for time integration
   ...
   case 'ode15s'
      ...
      % if (d == 3)
      %   [~,xinew]=ode15s_modified(@MovMesh_rhs,[tspan(1),tspan(end)],xi,opts,X, ...
      %                 M,tau,tri,tri_bf,bfE,nodes_fixed,G_fun); 
      % else
   ...
   case 'ode45'
      ...
      [~,xinew]=ode45(@MovMesh_rhs,[tspan(1),tspan(end)],xi,opts,X,M,tau, ...
                       tri,tri_bf,bfE,nodes_fixed,G_fun);
      Xinew = reshape(xinew(end,:)', d, Nv)';
   end
...
function F = MovMesh_rhs(t,xi,X,M,tau,tri,tri_bf,bfE,nodes_fixed,G_fun)
%
...
% modify forces at boundary points
   ...
   case 2
      ...
      if (fixed)
         ...
         vn = MovMesh_freeBoundary_vertexNormal(XI,tri,tri_bf,bfE);
...
function JAC = MovMesh_jac(t,xi,X,M,tau,tri,tri_bf,bfE,nodes_fixed,G_fun)
%
